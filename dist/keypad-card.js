...
function we(e) {
	...
	let n = e;
	for (let e of Object.keys(n)) xe.has(e) || t.push(`unknown option: ${e}`);
	let r = n.entity;
	...
	(typeof i != "string" || !be.has(i)) && t.push("action must be one of disarm, arm_home, arm_away, arm_night, arm_vacation");
	let a = n.title;
	...
	let s = X(n.min_length, 4, 1, 16, "min_length", t), c = X(n.max_length, 8, 1, 16, "max_length", t);
	s > c && t.push("min_length cannot exceed max_length");
	let l = X(n.clear_after_ms, 3e4, 0, 6e5, "clear_after_ms", t), u = X(n.key_size, 96, 48, 200, "key_size", t), d = n.layout ?? "auto";
	return (typeof d != "string" || !Se.has(d)) && t.push("layout must be auto, portrait, or landscape"), t.length > 0 ? { errors: t } : {
		errors: t,
		config: {
...
var Q, Te = [
	...
	constructor(...e) {
		super(...e), this.errors = [], this.code = "", this.busy = !1, this.notice = null, this.clearTimer = null;
	}
	setConfig(e) {
		let t = we(e);
	...
	disconnectedCallback() {
		super.disconnectedCallback(), this.stopClearTimer(), this.code = "";
	}
	stopClearTimer() {
	...
	render() {
		if (this.errors.length > 0) return I`<ha-card>
        <div class="errors">keypad-card configuration:\n${this.errors.join("\n")}</div>
      </ha-card>`;
		let e = this.config;
		if (!e) return R;
		let t = this.hass?.states[e.entity] === void 0;
		return I`<ha-card style="--kp-key: ${e.key_size}px">
      <div class="body ${e.layout}">
        <div class="panel">
          ${e.title ? I`<div class="title">${e.title}</div>` : R}
          ${t ? I`<div class="notice error">${e.entity} is not available</div>` : R}
          <div class="display" aria-label="code entry">${"•".repeat(this.code.length)}</div>
          ${this.notice ? I`<div class="notice ${this.notice.kind}">${this.notice.text}</div>` : R}
        </div>
        <div class="grid">
          ${Te.map((e) => I`<button
              type="button"
...
Q = $, Q.styles = o`
  ...
  `, Z([J({ attribute: !1 })], $.prototype, "hass", void 0), Z([Y()], $.prototype, "config", void 0), Z([Y()], $.prototype, "errors", void 0), Z([Y()], $.prototype, "code", void 0), Z([Y()], $.prototype, "busy", void 0), Z([Y()], $.prototype, "notice", void 0), customElements.get("keypad-card") || customElements.define("keypad-card", $), window.customCards = window.customCards ?? [], window.customCards.some((e) => e.type === "keypad-card") || window.customCards.push({
	type: "keypad-card",
	name: "Keypad Card",
